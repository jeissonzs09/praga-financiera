...
class ClienteController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Convertir arrays a JSON
        foreach (['tipo_identificacion','nivel_ingreso','ingresos','garantia'] as $campo) {
            $data[$campo] = json_encode($request->{$campo} ?? []);
        }
        ...
        if($request->hasFile('identidad_img')){
            ...
            foreach($request->file('identidad_img') as $img){
                $identidadImgs[] = $img->store('clientes/identidad', 'public');
            }
        ...
        if($request->hasFile('fotos_garantias')){
            ...
            foreach($request->file('fotos_garantias') as $img){
                $fotosGarantias[] = $img->store('clientes/garantias', 'public');
            }
        ...
        // Subir nuevos contratos
        if($request->hasFile('contrato_pdf')){
            foreach($request->file('contrato_pdf') as $archivo){
                $nombreOriginal = time().'_'.$archivo->getClientOriginalName();
                $contratosExistentes[] = $archivo->storeAs('clientes/contratos', $nombreOriginal, 'public');
            }
        }
    ...
    {
        $cliente = Cliente::findOrFail($id);
        return view('clientes.print', compact('cliente'));
    }
}